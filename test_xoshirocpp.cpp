// test_xoshirocpp.cpp — XoshiroCpp.hpp (C++17) 确定性单元测试
...
int main()
{
	...
	// ===== 扩展便捷 API =====
	{
		...
		for (int i = 0; i < 100; ++i)
			assert(RandPoisson() >= 0);

		// RandGamma：伽马分布（验证正数）
		for (int i = 0; i < 100; ++i)
			assert(RandGamma() > 0.0);

		// RandBits：N 位随机数
		for (int i = 0; i < 100; ++i)
		{
			assert(RandBits<8>() < 256);
			assert(RandBits<1>() < 2);
		}
